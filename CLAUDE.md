...
- **System self-knowledge** lives in the MongoDB Atlas `knowledge` database (Knowledge repo, `scripts/query.sh`) — its collections mirror the grid's five supers (application.* diamond core, workflow.* triangle, and atomic knowledge / data / architecture). Plans live in the Instruction repo (`PIVOT-PLAN.md`); decisions live in git history, their rationale in the knowledge base.
